...
use App\Exports\OrganizationDepositPaymentExport;

class AdminOrganizationDepositPayments extends Component
{
    use WithPagination;
    ...
    public function exportAll(){
        return Excel::download(new OrganizationDepositPaymentExport($this->search, $this->status, $this->start_date, $this->end_date), 'organization_deposit_payment_history.xlsx');
    }

    public function viewReceipt($paymentId)
    {
        $payment = OrganizationDepositPayment::with('capturedByAdmin')->findOrFail($paymentId);
    ...
    public function render()
    {
        ...
        $query = OrganizationDepositPayment::with('organization')
        ...
        return view('livewire.admin.admin-organization-deposit-payments', [
            'payments' => $payments,
            'totals'   => $this->totals,
        ]);
    }
